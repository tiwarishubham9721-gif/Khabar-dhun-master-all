...
def automation_cycle():
    try:
        seed_self_promotion(); seed_festival_promotions(datetime.now(timezone.utc).year)
    except Exception: pass
...
@app.get('/api/clock')
def api_clock():
    return {'timezone':'Asia/Kolkata','utc':NOW(),'display':datetime.now(timezone(timedelta(hours=5,minutes=30))).strftime('%d-%m-%Y %I:%M:%S %p')}
...
def public_site(request:Request, category:str=''):
    ...
    festival=c.execute("SELECT * FROM festival_promotions WHERE festival_date BETWEEN date('now') AND date('now','+7 day') ORDER BY festival_date LIMIT 1").fetchone(); promo=c.execute("SELECT * FROM promo_rotation WHERE active=1 ORDER BY priority DESC,id LIMIT 1").fetchone(); c.close(); return templates.TemplateResponse(request=request, name='public_site.html', context={'brand':b,'posts':posts,'category':category,'festival':dict(festival) if festival else None,'promo':dict(promo) if promo else None,'now':datetime.now(timezone(timedelta(hours=5,minutes=30))).strftime('%d-%m-%Y %I:%M:%S %p')})

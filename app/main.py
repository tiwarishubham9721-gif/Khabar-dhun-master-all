...
def public_site(request:Request):
    c=db(); b=dict(c.execute('SELECT * FROM brand WHERE id=1').fetchone()); posts=[dict(x) for x in c.execute('SELECT * FROM news WHERE status="APPROVED" ORDER BY id DESC LIMIT 30')]; c.close(); return templates.TemplateResponse(request=request, name='public_site.html', context={'brand':b,'posts':posts})
